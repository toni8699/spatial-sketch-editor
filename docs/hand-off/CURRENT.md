...
- **P23.0 stage-6 flip EXECUTED 2026-09-10 on branch `p23-stage6-flip` (owner go-ahead; commit `455f587`, awaiting merge to `main`)**: `LAYOUT_MUTATION_POLICY['wall-first']` flipped `disabled → adapted` in `apps/editor/src/lib/editor/store/document-format-policy.svelte.ts` (refusal reason retired to `null`); the two pre-flip gate-boundary pinning suites updated to the post-flip contract (`p23-f0-stage1-format-policy.test.ts` wall-first adapted; `p23-f0-stage2-writers.test.ts` flip assertion). Full gate green on the branch: Vitest 211 files / 2801 tests (+1 skipped, pre-existing), `layout-core` + `project-model` tsc clean, editor + museum svelte-check 0/0.
- **Stage-6 safety fix (review round, 2026-09-10, same branch, uncommitted on top of `455f587`)**: the flip had retired the pre-flip cross-format transaction rejection (commit re-check only refused never-adapted formats). The original invariant — begin format = live format = committed-snapshot format, all adapted — is now enforced explicitly: `beginLayoutTransaction` captures the begin format in `#openLayoutTransactionFormat`; `commitLayoutTransaction` classifies the live host AND the candidate snapshot (`snapshot.project.layout ?? snapshot`), refuses any mismatch or non-adapted format with `cancel()` + `'Layout format changed mid-transaction — edit refused'` (no history entry, no leaked bracket); `cancelLayoutTransaction`/`clearSharedHistory`/`importDocument` clear the captured format. Regression matrix added: legacy→wall-first, wall-first→legacy, wall-first→wall-first (commits), adapted→unrecognized. Guard-wiring scan updated to pin the commit re-check's policy consultation + dedicated refusal message.
- **F0 acceptance passed 2026-09-10 on `main`** (end-to-end: full Vitest gate + `layout-core`/`project-model` tsc + editor/museum `svelte-check` + agent-browser smoke on `/`, `/project/:id/spatial` incl. Plan/3D + Preview takeover/exit, `/museum`, `/museum/editor`; console shows only the known `cameraPlan` `ownership_invalid_mutation` warnings). Checklist items 1–22 pass against stages 1–5 + review fixes; stage-6 flip executed on the branch above, pending owner merge decision.
- **F0 review fixes committed 2026-09-09 on `main`** (`442f85a`, pre-flip must-fix + should-fix): scene `unrecognized` fail-closed + scene commit re-check with commit-swap tests (stage 1); sweep regression tests + directional-light omit fix + node-guard unification (stage-2 probe: claimed waypoint/anchor/keyframe hole did NOT reproduce — post-parse sweep already rejects); rotation-with-direction + survivor/reverse/edge subdivision + full-layout Save identity (stage 3); legacy composer-vs-composer parity + true-compat serialize identity (stage 4); connection-path adversarial passthrough + portal path unification + `faceKey` rejection field (stage 5); plan/code name-drift corrected. All writes still disabled; stage-6 flip NOT started.
...
- **Stage-6 flip + safety fix are DONE on branch `p23-stage6-flip`** (commit `455f587` + the uncommitted safety-fix delta): wall-first layout mutation is `adapted`; the cross-format transaction invariant is enforced explicitly at commit. Next: owner merges the branch to `main`, then P23.1 (already implementation-ready) starts with a targeted post-F0 seam recheck — no new research. In parallel, P24 planning runs the targeted post-F0 recheck toward the R9 minimum freeze; no P24 implementation until the accepted P23 minimum, and no P25 implementation yet.
...
- Review-fix full gate on the dirty tree immediately before commit: Vitest 211 files / 2801 tests passed (+1 skipped file/suite, pre-existing), `layout-core` + `project-model` tsc clean, editor + museum svelte-check 0/0.
