...
- Test-auth seam complete (uncommitted): `POST /test-auth/session` (Bearer `E2E_TEST_AUTH_SECRET` + `{user}` → canonical `google:e2e-*` session, allowlist `agent-admin`/`agent-user-a`/`agent-user-b`); route registered only when constructed with the secret (absent in prod → 404); editor `loginAs` helper under `tests/e2e/`; API suite 39 green (incl. 6 new: seam session, secret/identity rejection, structural absence, config fail-closed, real-Postgres cross-ownership), editor suite 2568 green, both `check`s clean.
