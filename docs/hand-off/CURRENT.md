...
- **P23.0 F0 stage 5**: standalone Scene import frame mapping, portal Save-blocker, no-second-transform regression at the museum/visitor seams, then gate + commit. **Stop at the F0 boundary; do not run the stage-6 flip / enable new-schema writes.** — each committed. **Stop at the F0 boundary; do not run the stage-6 flip / enable new-schema writes.**

## Verification

- Stage-4 full gate on the dirty tree immediately before commit: Vitest 210 files / 2775 tests passed (+1 skipped file/suite, pre-existing), `layout-core` + `project-model` tsc clean, editor + museum svelte-check 0/0.
