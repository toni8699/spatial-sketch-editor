...
- **P23.0 stage-6 flip (owner-gated, NOT started)**: F0 acceptance passed 2026-09-10 (checklist items 1–22 per the P23.0 execution-order addendum; writers verified still disabled via `document-format-policy.svelte.ts`). Next is enabling new-schema Save/write paths — requires explicit owner go-ahead; do not run unasked.
