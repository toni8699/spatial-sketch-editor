...
- **P23.0 F0 gate review**: stages 1-5 + review fixes committed; all writes still disabled. Next is the owner-gated stage-6 flip (enable new-schema writes) plus the F0 acceptance checklist pass — NOT started. **Stop at the F0 boundary; do not run the stage-6 flip / enable new-schema writes.** — each committed. **Stop at the F0 boundary; do not run the stage-6 flip / enable new-schema writes.**
